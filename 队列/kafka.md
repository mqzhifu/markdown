...
| 提交类型    | 提交时机          | 优点           | 缺点                   | 适用场景               | 重复消费 |
| ------- | ------------- | ------------ | -------------------- | ------------------ | ---- |
| 自动提交    | 定时周期提交        | 代码极简         | 宕机极易丢消息、乱序           | 测试环境、日志埋点非核心业务     | 极高   |
| 手动同步提交  | 阻塞等待提交完成      | 提交结果明确，失败可重试 | 略微降低吞吐               | 订单、支付、IM 消息等强一致性业务 | 极低   |
| 手动异步提交  | 不阻塞提交         | 吞吐高，不阻塞      | 提交失败无即时重试，可能丢 offset | 高吞吐、允许少量重复的业务      | 低    |
| 异步带回调提交 | 异步提交 + 回调监听结果 | 兼顾高吞吐 + 失败感知 | 需自行实现失败重试逻辑          | 大数据流式批量消费          | 低    |
| 批量手动提交  | 批量提交          | 减少网络 IO，提升性能 | 批量内单条失败易引发批量重消费      | 批量入库、统计类业务         | 中等   |
...
每个 broker  启动后，会去 zk 中 set 一个临时值，如果能 set 成功就是 controller。
