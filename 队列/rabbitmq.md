...
| key          | value                                   |     |
| :----------- | :-------------------------------------- | --- |
| Virtual host | 类似 namespace，一个实例上，为不同业务划分不同的实例范围       |     |
| connect      | tcp 连接                                  |     |
...
新建一个队列A，给设置一个死信交换机，再绑定一个新的队列B
上面失败的消息就会进入死信交换机的死信队列中

队列A永远不要去消费，所有的延迟消息也都发给队列A，并且强制要求消息要设置TTL值。当队列A的消息的TTL到期后，就会转到队列B，此时再开consumer 消费

也算是间接实现了：延迟这个功能
缺点：
假设消息队列，第一个消息，失效时间是10秒，第二个消息失效时间是5秒，但RABBITMQ只检查第一个消息，10秒后，才变成死信消息，但第二个消息其实早早就超时了。



最好是用它官方的延迟插件：
还是投递延迟消息，但这时，并不直接进队列，而是先进入 Mnesia 数据库暂存，到时间了才到队列中。
...
每台机器都会保存元数据： exchange  queue  元/基础 配置信息
