...
客户端不要写死IP，要订阅  哨兵的 频道，从这里动态拿主库IP
...
某一个节点如果挂了，且没有从库，那整个集群全挂


Redis集群中有 16384 个哈希槽
...
我想某个KEY就落到特定的机器上：
crc16 只计算key 中   {} 包含的值：{goods_1001}
商品库存 Key：seckill:stock:{goods_1001}:1


client 与任意一台 REDIS 连接后，能拿到 集群信息
比如：client 连接 A，能拿到B、C，会再与B、C 再建立连接。
之后，当一个key 写入时，client 在本地计算出属于 A B C 哪个，就能确定落在哪台机器上
...
这两个并不冲突，集群中操作的是：master ，从可以从master 中同步数据 
如果 某个master 挂了，从库可以立刻提上来