...
解决办法：return 后面一定加 x = append(x,x)//返回的是最新的切片

COPY函数，是一层深copy，如果要复制的结构体，还有子层是复杂类型，它就是浅copy了
...
数组清空操作：
```
//切片长度变为 0，容量不变，仍然持有底层数组引用，内存不会释放。但可复用
s[:0]

切片变为 nil，不指向任何底层数组，引用被释放，无引用时数组会被 GC 回收。
s = nil
```
...
写操作：如果KEY存在，直接走CAS，不用加锁。否则 ，加 Mutex 锁，避免多协程同时写
读操作，使用的是原子操作  atomic.Value，也就是并发读，在底层CPU也会变成串行读，代码层不用加读锁了
