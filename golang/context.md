...
value  + cacel + timer  3种context 是可以混在一起用的。

context 的核心就是上下文，不可能，一个函数定义了N种context ，比如：我想用value + cancel ，那参数就得定义两个。

不同类型的context 可以叠加使用。

如果，context 叠加非常多的子context ，出现问题：
- 在实际使用 context 要一直向父节点查找，它是链表结构，内存不连续，查找性能很差
- 是多层叠加，其中的变量如果外部一直持使用，会内存泄露
- CPU无法命中L1/L2缓存


注意：
- 不太嵌入层数太多，如果一定要这么做，最好在函数中加参数
- 不要在 value 里存大的数据结构
