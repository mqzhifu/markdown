...
因为X返回的是指针，被迫在堆开辟内存空间。第一次GC，  x 会被标记成灰色，再变成白色，不会被清理。第二次GC，因为main 没有人使用这个 x，所以它一直是白色的。
写保护：假设 当前 a 被标记成了灰色，扫描时，把 a 的子连接 d c 放入灰色池子，此时，a 变成 黑色。但此时，程序员突然执行了a->c，c是白色，原本跟c连接的也不知道是谁，又因为a已经变成黑色处理过了，c 现在就是白色，很容易被误杀。
如果发生在GC期间，改变了变量的连接，就把C直接扔到灰色池子里
