...
不允许做其它的操作：
- C/C++里的指针可以做 PP++ PP-- 操作不同。GO里不允许。
- 给指针强制转换类型

>如果一定要对指针进行计算需要： unsafe.Pointer + uintprt

# unsafe.Pointer 和 uintprt
...
uintprt：一种无符号整型类型，可以保存一个指针值，也可以进行指针运算，
...
unsafe.Pointer 可以打破GO里的：指针安全限定，拿到地址 。再给到 uintprt 就可以运算了

unsafe.Pointer + uintprt = C语言里对指针的操作


```
strZeroCopy := *(*string)(unsafe.Pointer(&a))
```
...
```
var p *int = nil; //内存中会开辟 一个int空间，内容都是0
var p = make(int)
