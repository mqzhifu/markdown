...
cache = map+rwLock+TTL
map = map + rwlock

自己写 map  可能有遗漏，使用官方的效率更高。

另外高级点的cache:
- 还有LRU，即一个KEY长时间未使用，会自动KILL
- 给 key 分片 shard ，不是使用全局锁，只锁部分数据。比如：一个MAP拆分成10个小MAP
- GC ，map[string]offset ，map 不存真正的值 ，这样GC 遍历MAP效率更高
...
核心原理：select 会检查当前的多个 case 有没有可执行，如果都没有，因为每个case 都会监听的是一个 chanel，把自己这个 case 注册到这个channe l等待队列中，然后，当这个chanel 发生事件，就可以唤醒这个G
