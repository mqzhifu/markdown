...
任何3方中间件，都会有网络请求，都有可能阻塞当前应用程序。如：
- DBMS/KVDB
- REDIS/MEMCACHE
- 支付
...
情况2：不知道调用几个服务 ，实际只是发出一个请求，但这个请求又依赖其它微服务.....

情况3：提前知道N个服务调用 ，但有顺序，不能多协程请求


使用 context + timer 模式，能保证 ：当前卡顿的程序立即返回（卡顿的调用还是会执行的）


要实时分析 链路追踪，找出请求过多的，看看，能不能新写一个：批量处理的接口
