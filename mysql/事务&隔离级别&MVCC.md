...
| 幻读    | 读取 已提交的数据 | 两次 select count 的值不一样 范围查找 | insert，增加数据 |
| 不可重复读 | 读取 已提交的数据 | 两次 select 的字段值不一样          | update，某条数据 |

## 事务隔离级别

从上面3个问题来看，SQL 标准提出了四种隔离级别来规避这些现象
隔离级别越高，性能效率就越低，这四个隔离级别如下：
...
读未提交 和 读已提交 不详细说了，基本不太可能使用。

### Repeatable Read（可重读）

mySql 的默认事务隔离级别

两次读取操作，可能影响结果的操作：其它事务操作
- update 某个字段值
- 其它事务 delete/insert

读已提交数据：能解决 其它事务 udpate 某个字段值 ，但解决不了幻读

解决幻读：MVCC（快照读+当前读）+间隙锁，
解决不可重复读：MVCC（快照读+当前读）
...
>就是：你读你的，其它事务仍然可以 update 数据，不影响

当前读：读取最新版本，会加排它锁。 update delete insert
>这时：你读的就是最新的，且不让其它事务读了
...
间隙锁：为了防止在“当前读”的时候别人突然插空塞进来一条新数据（导致幻读）
阻止其他事务在这个间隙里执行 INSERT 插入新数据。
